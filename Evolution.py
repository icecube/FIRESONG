...
def get_evolution(evol):
    evolutions = { "NoEvolution": Evolution_NoEvolution,
                   "HB2006SFR": Evolution_HopkinsBeacom2006StarFormationRate,
                   "YMKBH2008SFR": Evolution_YukselMatthewKistlerBeacomHopkins2008StarFormationRate,
                   "CC2015SNR": Evolution_CandelsClash2015SNRate,
                 }
    if not evol in evolutions.keys():
        raise NotImplementedError("Source evolution " + evol + " not implemented.")
...
class Evolution_NoEvolution(Evolution):
    def parametrization(self, x):
        return 1.
  
class Evolution_HopkinsBeacom2006StarFormationRate(Evolution):
...
class Evolution_YukselMatthewKistlerBeacomHopkins2008StarFormationRate(Evolution):
...
class Evolution_CandelsClash2015SNRate(Evolution):
    def parametrization(self, x):
        a = 0.015
        b = 1.5
        c = 5.0
...
class SourcePopulation():
    ...
    def RedshiftDistribution(self, z):
        ...
        return 4*np.pi* self.evolution(np.log10(1.+z)) * cosmolopy.distance.diff_comoving_volume(z, **self.cosmology)
    ...
    def Nsources(density, zmax):
        ...
        """
        vlocal = cosmolopy.distance.comoving_volume(self._zlocal, **self.cosmology)
        Ntotal = density * vlocal / (self.RedshiftIntegral(zlocal) / 
                                     self.RedshiftIntegral(zmax) )
        return Ntotal
        
    def Flux2Lumi(fluxnorm, index, emin, emax, E0=1e5):
        """
        ...
        fluxnorm units are []
        """
        flux_integral = scipy.integrate.quad(lambda E: E*(E/E0)**(-abs(index)), emin, emax)[0]
        luminosity = fluxnorm / E0**2. * flux_integral * self.GeV_per_sec_2_ergs_per_year * 4. * np.pi * (self.dL1*self.Mpc2cm)**2. 

        return luminosity    
        
    def Lumi2Flux(luminosity, index, emin, emax, E0=1.e5):
        """  
        $$ L_\nu = \frac{ \Phi_{z=1}^{PS} }{E_0^2} \int_{E_\mathrm{min}}^{E_\mathrm{max}} E \left(\frac{E}{E_0}\right)^{-\gamma}\,\mathrm{d}E\,4\pi d_L^2(z=1) $$
        ...
        """
        flux_integral = scipy.integrate.quad(lambda E: E*(E/E0)**(-abs(index)), emin, emax)[0]
        fluxnorm = luminosity / 4. / np.pi / (self.dL1*self.Mpc2cm)**2. / self.GeV_per_sec_2_ergs_per_year / flux_integral * E0**2. 
        return fluxnorm
 
    def StandardCandleSources(fluxnorm, density, zmax, index):
        ...
        Fluxnorm = all_sky_flux / Ntotal / self.dL1**2. / scipy.integrate.quad(lambda z: (1.+z)**(-abs(index)+2) / (self.LuminosityDistance(z)**2.) * self.RedshiftDistribution(z) / norm, 0,10.)[0]
...
class TransientSourcePopulation(SourcePopulation):
    ...
    def StandardCandleSources(fluxnorm, density, zmax, index):
        ...
        fluence = all_sky_flux / Ntotal / self.dL1**2. / scipy.integrate.quad(lambda z: (1.+z)**(-abs(index)+3) / (self.LuminosityDistance(z)**2.) * self.RedshiftDistribution(z) / norm, 0,10.)[0]
